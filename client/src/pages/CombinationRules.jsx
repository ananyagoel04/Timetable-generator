...
export default function CombinationRules() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {loading ? <p className="text-dark-400 text-center py-12">Loading...</p> : rules.length === 0 ? (
        <div className="glass-card p-12 text-center">
          <Layers size={48} className="mx-auto text-dark-500 mb-4" />
          <h2 className="text-lg font-bold text-white mb-2">No Combination Rules</h2>
          <p className="text-dark-400 text-sm">Combine multiple classes for shared subjects like Library, Games, or common electives.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {r.appliesTo?.map((a, i) => (
                      <span key={i} className="badge bg-dark-700 text-dark-300 border border-dark-600 text-[10px]">{a.class?.name || '?'}</span>
                    ))}
                  </div>
                  {/* Live Preview */}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => { setEditing(r); setSelectedClasses(r.appliesTo?.map(a => a.class?._id || a.class) || []); setForm({ name: r.name, subject: r.subject?._id, teacher: r.teacher?._id, room: r.room?._id || '', periodsPerWeek: r.periodsPerWeek, strictness: r.strictness }); setModalOpen(true); }} className="p-1.5 rounded-lg hover:bg-dark-700 text-dark-400 hover:text-white"><Edit2 size={14} /></button>
                  <button onClick={() => handleDelete(r._id)} className="p-1.5 rounded-lg hover:bg-red-500/20 text-dark-400 hover:text-red-400"><Trash2 size={14} /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Rule' : 'New Combination Rule'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div><label className="text-xs text-dark-400 mb-1 block">Rule Name</label><input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} required className="input-field" placeholder="e.g. Library for 9A & 9B" /></div>
          ...
          <div>
            <label className="text-xs text-dark-400 mb-2 block">Classes to Combine</label>
            <div className="flex flex-wrap gap-2">
              {classes.map(c => (
                <label key={c._id} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border cursor-pointer transition-colors ${selectedClasses.includes(c._id) ? 'bg-primary-500/20 border-primary-500/40 text-primary-400' : 'bg-dark-800 border-dark-700 text-dark-300 hover:border-dark-500'}`}>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div><label className="text-xs text-dark-400 mb-1 block">Room</label><select value={form.room} onChange={e => setForm(f => ({...f, room: e.target.value}))} className="select-field"><option value="">Auto</option>{rooms.map(r => <option key={r._id} value={r._id}>{r.name}</option>)}</select></div>
            <div><label className="text-xs text-dark-400 mb-1 block">Periods/Week</label><input value={form.periodsPerWeek} onChange={e => setForm(f => ({...f, periodsPerWeek: +e.target.value}))} type="number" min={1} className="input-field" /></div>
            <div><label className="text-xs text-dark-400 mb-1 block">Strictness</label><select value={form.strictness} onChange={e => setForm(f => ({...f, strictness: e.target.value}))} className="select-field"><option value="must_combine">Must Combine</option><option value="try_combine">Try to Combine</option><option value="combine_if_possible">If Possible</option></select></div>
          </div>
