...
export default function PlatformAdmin() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all
                ${tab === t.id ? 'bg-primary-600 text-slate-900 dark:text-dark-50 shadow-lg shadow-primary-600/30' : 'text-slate-500 dark:text-dark-400 hover:text-slate-900 dark:hover:text-dark-50 hover:bg-slate-100/60 dark:hover:bg-dark-800/60'}`}>
      ...
      {loading ? <div className="text-center py-16 text-slate-500 dark:text-dark-400">Loading...</div> : (
        <>
          {tab === 'schools' && (
            <div className="space-y-3">
              <p className="text-sm text-slate-500 dark:text-dark-400">{schools.length} school{schools.length !== 1 ? 's' : ''} on platform</p>
              {schools.map(s => (
                <div key={s._id} className="glass-card p-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-dark-500">{new Date(s.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {tab === 'audit' && (
            <div className="space-y-2">
              <p className="text-sm text-slate-500 dark:text-dark-400">Recent platform activity</p>
              {auditLogs.length === 0 ? <div className="glass-card p-12 text-center text-slate-400 dark:text-dark-500">No audit logs yet</div> :
                auditLogs.slice(0, 30).map((log, i) => (
                  <div key={log._id || i} className="glass-card p-3 flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-white dark:bg-dark-800 flex items-center justify-center"><Activity size={14} className="text-slate-500 dark:text-dark-400" /></div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-slate-900 dark:text-dark-50 font-medium truncate">{log.action}</p>
                      <p className="text-[10px] text-slate-400 dark:text-dark-500">{log.entity} · {log.performedBy || 'system'} · {new Date(log.createdAt).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
            </div>
          ...
          {tab === 'users' && (
            <div className="space-y-3">
              <p className="text-sm text-slate-500 dark:text-dark-400">Platform-level users</p>
