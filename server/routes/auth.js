...
/**
 * Auth router factory — receives authLimiter from server.js
 * authLimiter is ONLY applied to login/register/forgot/reset (brute-force protection)
 * /me, /refresh, /switch-school are NOT rate-limited aggressively
 */
module.exports = function createAuthRouter(authLimiter) {
  ...
  router.post('/register', authLimiter, validateRegister, async (req, res, next) => {
    try {
      ...
      // Only auto-attach to a school if one is explicitly provided (e.g. invitation link)
      const schoolId = req.body.schoolId || req.query.schoolId;
      let school = null;
      let session = null;
      if (schoolId) {
        school = await School.findById(schoolId);
        session = school ? await AcademicSession.findOne({ school: school._id, isCurrent: true }) : null;
      }
  ...
  // GET /api/auth/me — NO strict rate limit (normal navigation uses this)
  router.get('/me', protect, async (req, res) => {
    const user = await User.findById(req.user._id).populate('schools.school activeSchool activeSession');
    res.json({ success: true, data: user });
  });

  // PUT /api/auth/switch-school — NO strict rate limit
  ...
  // POST /api/auth/refresh — NO strict rate limit
  ...
  router.post('/forgot-password', authLimiter, validateForgotPassword, async (req, res, next) => {
    try {
  ...
  router.post('/reset-password', authLimiter, validateResetPassword, async (req, res, next) => {
    try {
